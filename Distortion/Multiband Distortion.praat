...
procedure applyDistortion: .drive, .type, .gain
    # Convert to strings for formula (procedure params need string interpolation)
    .d$ = string$(.drive)
    .g$ = string$(.gain)
    .ng$ = string$(-.gain)
    ...
    if .type = 1
        # Soft Clip (Tanh)
        Formula: "tanh(self * " + .d$ + ") * " + .g$
        
    elsif .type = 2
        # Hard Clip - use nested if/then/else (no elsif in formula language)
        .input$ = "(self * " + .d$ + ")"
        Formula: "if " + .input$ + " > 1 then " + .g$ + " else (if " + .input$ + " < -1 then " + .ng$ + " else " + .input$ + " * " + .g$ + " fi) fi"
        
    elsif .type = 3
        # Sine Fold
        Formula: "sin(self * " + .d$ + ") * " + .g$
    endif
endproc
