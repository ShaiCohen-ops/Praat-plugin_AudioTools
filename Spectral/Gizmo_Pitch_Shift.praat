...
#
# Changelog v1.0 (supersedes the v0.1 draft; the audio is IDENTICAL
# to v0.1 to floating-point precision - this release changes speed
# and interface only):
#
#   CRITICAL 1 - the inner loop was interpreted, not vectorised.
#     v0.1 walked every FFT bin from the script: two Praat queries
#     (Get real value in bin / Get imaginary value in bin) plus up to
#     two setters (Set value at sample number) per bin, per frame,
#     per channel. At the 0.08 s default on 44.1 kHz that is
#     nBins = 2049, so ~8200 interpreter round-trips per grain and
#     ~43 grains per second of audio per channel - on the order of
#     10^7 interpreted operations for a 30 s stereo file.
#     v1.0 keeps the identical algorithm but expresses every per-bin
#     step as a Formula over an nBins-sample Sound buffer, so the
#     loop runs inside Praat's C evaluator. Per-frame interpreter
#     cost drops from ~8200 calls to ~20. The FFT pair is now the
#     dominant cost, which is where it belongs.
#
#   2 - The scatter was rewritten as a gather. The transposition
#     synMagn[round(k*ratio)] += magn[k] is a scatter, which no
#     elementwise Formula can express. Inverted: the source bins
#     contributing to one target bin span a window of width 1/ratio,
#     so over the -12..+12 st range at most four source bins can land
#     on one target. The gather runs as ceiling(1/ratio)+2 "lanes",
#     one Formula each, every lane masked by the same
#     round(k*ratio) test the scalar version used.
#
#   3 - The weight (COLA) buffer does not depend on the audio, so it
#     is accumulated once and reused for every channel. v0.1 rebuilt
#     it per channel.
#
#   4 - The re/im staging buffer is gone. The target Spectrum is
#     stamped by one Formula reading the magnitude and accumulated
#     phase buffers, with DC and Nyquist imaginary parts zeroed by a
#     (col > 1) * (col < nBins) mask instead of two element setters.
#
#   5 - Presets, visualization and Play added. Geometry, COLA ripple
#     and a measured-vs-requested pitch check are reported.
#
# Description:
#   Offline FFT phase-vocoder pitch shifter using the spectral bin
#   transposition mechanism of Max/MSP's gizmo~:
#
#     k_target = round (k_source * ratio),  ratio = 2 ^ (st / 12)
#
#   Magnitudes are summed into the target bin. The instantaneous bin
#   frequency is recovered from the inter-frame phase difference,
#   scaled by the same ratio, and re-integrated into a per-target-bin
#   synthesis phase accumulator - this is what stops the transposed
#   partials from cancelling at frame boundaries.
#
#   Pure Praat: no Python, no external binaries, no extensions.
#
#   FFT SIZE: the requested frame length is snapped to the nearest
#   power of two. "To Spectrum: yes" zero-pads to a power of two and
#   "To Sound" returns the PADDED length, so an unsnapped frame makes
#   the overlap-add index arithmetic silently wrong. Snapping keeps
#   frame, FFT and resynthesised grain all at exactly nGrain samples.
#   The report prints requested vs actual.
#
#   GAIN: unity gain comes from a parallel sum-of-w^2 buffer that the
#   output is divided by - not from assuming the window satisfies
#   COLA at the chosen hop, which is why the Gaussian option is safe
#   at any hop. NO peak normalization is applied anywhere, so the
#   output level is the level the algorithm produced and the spectrum
#   panel is a fair comparison.
#
#   CHANNELS: every channel is processed independently and the
#   original channel count, start time and duration are restored.
#   Each channel is extracted to mono first - To Spectrum crashes on
#   multi-channel input.
#
