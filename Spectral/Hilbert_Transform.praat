...
# Version: 0.4 (2026) - Draw the reversed envelope overlay (was never plotted)
# License: MIT License
#
# Description:
...
form Hilbert Time-Reversed Envelope v0.4
    optionmenu Preset: 1
        option Custom
...
clearinfo
writeInfoLine: "=== Hilbert Time-Reversed Envelope v0.4 ==="
...
if draw_visualization
    ...
    # original envelope (orange), explicit ranges so the reversed overlay
    # lines up exactly
    Colour: "{0.9, 0.5, 0.2}"
    Line width: 2
    Draw: 0, envDur, 0, yTop, "no", "Curve"

    # reversed envelope (green) - this is what actually gets applied. Make
    # a time-reversed copy and draw it on the SAME ranges.
    selectObject: envForVizID
    revVizID = Copy: "env_viz_rev"
    Formula: "self[ncol - col + 1]"
    Colour: "{0.2, 0.7, 0.4}"
    Line width: 1.5
    Draw: 0, envDur, 0, yTop, "no", "Curve"
    removeObject: revVizID

    Line width: 1
    Colour: "Black"
    Draw inner box
    Font size: 9
    Text top: "no", "Hilbert envelope (orange = extracted, green = reversed = applied)"
    Text left: "yes", "Env"
