...
# Version: 0.5 (2026) - DC-band fix (audio change), honest filter viz, title axes
#
# v0.5 note: earlier versions also "boosted" the band around
# 0 Hz, because round(x/f0) = 0 satisfied the harmonic test.
# The comb now starts at harmonic 1; DC/subsonic content is
# attenuated like any non-harmonic region. This CHANGES THE
# SOUND relative to v0.4, mainly for sources with rumble or
# DC offset (and overall level via peak normalization).
# License: MIT License
#
# Description:
...
# valid only for harmonic index round(x/f) >= 1 (so the DC
# band is NOT treated as a harmonic). Inside the band: boost;
# otherwise: attenuate (split at the mid-frequency cutoff).
selectObject: matrixL_ID
Formula: "if round(x / " + fundStr$ + ") >= 1 and abs(x - round(x / " + fundStr$ + ") * " + fundStr$ + ") < " + bwL$ + " then self * " + boostStr$ + " else (if x < " + midCutStr$ + " then self * " + lowAttStr$ + " else self * " + highAttStr$ + " endif) endif"
...
if create_stereo
    ...
    Formula: "if round(x / " + fundStr$ + ") >= 1 and abs(x - round(x / " + fundStr$ + ") * " + fundStr$ + ") < " + bwR$ + " then self * " + boostStr$ + " else (if x < " + midCutStr$ + " then self * " + lowAttStr$ + " else self * " + highAttStr$ + " endif) endif"
...
if draw_visualization
    ...
    # Draw the TRUE brick-wall gain curve: rectangular boost bands
    # at harmonics >= 1, baseline at low_mid_attenuation below the
    # mid cutoff and high_freq_attenuation above it.
    Line width: 2
    
    harmNum = 1
    harmFreq = fundamental_frequency
    prevRight = 0
    while harmFreq - harmonic_bandwidth < maxFreq
        leftEdge = harmFreq - harmonic_bandwidth
        if leftEdge < prevRight
            leftEdge = prevRight
        endif
        rightEdge = harmFreq + harmonic_bandwidth
        if rightEdge > maxFreq
            rightEdge = maxFreq
        endif
        
        # Baseline segment before this band (green)
        Colour: "{0.2, 0.7, 0.4}"
        @drawBaseline: prevRight, leftEdge
        
        # Boost band as a true rectangle (orange)
        if leftEdge < rightEdge
            Colour: "{0.9, 0.4, 0.2}"
            @baseGain: leftEdge
            Draw line: leftEdge, baseGain.v, leftEdge, harmonic_boost
            Draw line: leftEdge, harmonic_boost, rightEdge, harmonic_boost
            @baseGain: rightEdge
            Draw line: rightEdge, harmonic_boost, rightEdge, baseGain.v
        endif
    ...
    # Baseline after the last band (green)
    Colour: "{0.2, 0.7, 0.4}"
    @drawBaseline: prevRight, maxFreq
...
# Baseline (non-harmonic) gain at frequency .f: mirrors the
# attenuation branch of the audio Formula exactly.
...
# Draw the baseline gain from .f1 to .f2, stepping down at the
# mid-frequency cutoff if it falls inside the segment.
procedure drawBaseline: .f1, .f2
    if .f2 > .f1
        if mid_freq_cutoff > .f1 and mid_freq_cutoff < .f2
            Draw line: .f1, low_mid_attenuation, mid_freq_cutoff, low_mid_attenuation
            Draw line: mid_freq_cutoff, low_mid_attenuation, mid_freq_cutoff, high_freq_attenuation
            Draw line: mid_freq_cutoff, high_freq_attenuation, .f2, high_freq_attenuation
        else
            @baseGain: .f1
            Draw line: .f1, baseGain.v, .f2, baseGain.v
        endif
    endif
endproc
