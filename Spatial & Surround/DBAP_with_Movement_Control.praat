...
if draw_visualization
    ...
    Text: 0.5, "centre", -0.25, "half",
        ... soundName$
        ... + "  |  " + movementName$
        ... + "  |  " + configName$ + " (" + string$(numSpeakers) + " ch)"
        ... + "  |  " + fixed$(duration, 2) + " s"

    # ----------------------------------------------------------
    # Spatial map with trajectory (left panel)
    # ----------------------------------------------------------
    Select outer viewport: 0, 4.1, 0.75, 4.25
    Select inner viewport: 0.55, 3.85, 0.90, 4.10
    ...
    # Trajectory path (blue→red gradient)
    ...
    if maxTraj > 1
        ...
        for i from 2 to maxTraj
            ...
            rCol = prog
            bCol = 1 - prog
            Colour: "{" + fixed$(rCol, 2) + ", 0.30, " + fixed$(bCol, 2) + "}"
            Draw line: trajX[i - 1], trajY[i - 1], trajX[i], trajY[i]
        endfor
        Line width: 1
    endif
    ...
    # Speakers
    for sp from 1 to numSpeakers
        Paint circle (mm): "{0.25, 0.50, 0.72}", spkX[sp], spkY[sp], 3.5
        Font size: 5
        Colour: "White"
    ...
    # Start / end markers
    if maxTraj > 0
        Paint circle (mm): "{0.20, 0.50, 0.80}", trajX[1], trajY[1], 2
        Font size: 5
        Colour: "{0.15, 0.40, 0.65}"
        Text: trajX[1] + 0.10, "left", trajY[1], "half", "Start"
        Paint circle (mm): "{0.82, 0.28, 0.28}", trajX[maxTraj], trajY[maxTraj], 2
        Colour: "{0.65, 0.20, 0.20}"
        Text: trajX[maxTraj] + 0.10, "left", trajY[maxTraj], "half", "End"
    endif
    ...
    Draw inner box
    Font size: 7
    Text top: "no", "Movement path & speaker positions"

    # ----------------------------------------------------------
    # Parameters panel (right panel, upper)
    # ----------------------------------------------------------
    Select outer viewport: 4.1, 8, 0.75, 2.55
    Select inner viewport: 4.40, 7.65, 0.90, 2.42
    Axes: 0, 1, 0, 1
    Paint rectangle: "{0.96, 0.96, 0.96}", 0, 1, 0, 1

    Font size: 7
    Colour: "Black"
    Text: 0.02, "left", 0.88, "half", "##Parameters##"

    Font size: 6
    Colour: "{0.30, 0.30, 0.30}"
    Text: 0.05, "left", 0.70, "half", "Movement:  " + movementName$
    Text: 0.05, "left", 0.54, "half", "Speakers:  " + string$(numSpeakers) + " (" + configName$ + ")"
    Text: 0.05, "left", 0.38, "half", "Speed: " + fixed$(speed, 1) + "  |  Radius: " + fixed$(radius, 2)
    Text: 0.05, "left", 0.22, "half", "Rolloff: " + fixed$(rolloff, 1) + "  |  Chunk: " + fixed$(chunk_duration * 1000, 1) + " ms"

    Colour: "Black"
    Draw rectangle: 0, 1, 0, 1

    # ----------------------------------------------------------
    # Output waveform (right panel, lower)
    # ----------------------------------------------------------
    Select outer viewport: 4.1, 8, 2.65, 4.25
    Select inner viewport: 4.40, 7.65, 2.78, 4.15
    selectObject: result
    ...
    Text top: "no", "Output waveform (ch 1 blue, ch 2 orange)"

    # ----------------------------------------------------------
    # Input spectrogram (full width)
    # ----------------------------------------------------------
    Select outer viewport: 0, 8, 4.35, 5.75
    Select inner viewport: 0.55, 7.65, 4.45, 5.65
    selectObject: sound
    numChOrig = Get number of channels
    if numChOrig > 1
        Extract one channel: 1
    ...
    Font size: 7
    Colour: "Black"
    Text: 0.02, "left", 0.72, "half", "##" + soundName$ + "_DBAP_" + movementName$ + "_" + configName$ + "##"

    Font size: 6
    Colour: "{0.30, 0.30, 0.30}"
    Text: 0.02, "left", 0.28, "half",
        ... "Spkrs=" + string$(numSpeakers)
        ... + "  Traj=" + movementName$
        ... + "  Speed=" + fixed$(speed, 1)
        ... + "  Radius=" + fixed$(radius, 2)
        ... + "  Rolloff=" + fixed$(rolloff, 1)
        ... + "  Chunk=" + fixed$(chunk_duration * 1000, 1) + "ms"
        ... + "  " + fixed$(duration, 2) + "s @" + string$(sr) + "Hz"
