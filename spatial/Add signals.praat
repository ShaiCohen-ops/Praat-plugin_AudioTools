# Stereo Mixer with Two-Channel Gain Control
# Simplified Form: Uses minimal comments while retaining ch1 (Left) and ch2 (Right) gains.

# 1. Check for selected sounds and get Sound IDs
...
# 2. Form with simplified gain controls (Left Gain: ch1_i, Right Gain: ch2_i)
form Stereo Balance and Volume
    comment Enter volume multipliers (1.0=normal, 0.0=mute)
    
    comment --- Sound 1 (Left Gain / Right Gain) ---
    real ch1_1 1.0
    real ch2_1 1.0
...
# 3. Convert all sounds to stereo (required for separate L/R manipulation)
for i to numberOfSounds
    # Select sound by its stored ID
    select sound'i'
...
# 4. Create empty stereo sound as base
select sound1
duration = Get total duration
sampleRate = Get sampling frequency
resultSound = Create Sound from formula: "mixed_stereo", 2, 0, duration, sampleRate, "0"

# 5. Mix each sound with ch1/ch2 gains
for i to numberOfSounds
    
    # Select the sound ID and retrieve the name for logging
    select sound'i'
    name$ = selected$("Sound")

    # Process only the first 5 sounds (as defined in the form)
    if i <= 5
        # Retrieve the user-defined gains (ch1_i and ch2_i)
        leftGain = ch1_'i'
        rightGain = ch2_'i'
        ...
        # Use Formula (part) for applying gain to specific channels (1 and 2)
