...
tail_samples = round((endTime - covered_end) * sampleRate)
if keep_trailing_audio and tail_samples >= 2
    selectObject: sound
    tailSeg = Extract part: covered_end, endTime, "rectangular", 1, "no"
    has_tail = 1
    appendInfoLine: "Trailing remainder kept: ", fixed$((endTime - covered_end) * 1000, 1), " ms (forward)"
elsif tail_samples >= 2
    appendInfoLine: "Trailing remainder dropped: ", fixed$((endTime - covered_end) * 1000, 1), " ms"
endif
