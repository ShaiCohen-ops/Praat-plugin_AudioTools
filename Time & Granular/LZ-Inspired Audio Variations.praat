...
#
# Optimization:
#   - Vectorized arrays (eliminates selectObject overhead)
#   - Sort-and-sweep (enables early termination in comparisons)
#
# Changelog v0.2:
...
if variation_method = 1
    variation_name$ = "Pitch shift"
elsif variation_method = 2
    variation_name$ = "Time stretch"
elsif variation_method = 3
    variation_name$ = "AM"
elsif variation_method = 4
    variation_name$ = "Spectral filter"
...
# === Info ===
writeInfoLine: "=== LZ-Inspired Audio Variations ==="
...
# === SORT AND SWEEP: Sort for early termination ===
...
# Max acceptable difference for early break
if analysis_type = 1
...
# Store which window was used (for visualization)
...
if draw_visualization
    ...
    Font size: 12
    Colour: "Black"
    Text: 0.5, "centre", 0.5, "half", "LZ Audio Variations: " + sound_name$
    ...
    # Draw which source window was used for each output window
    for i to num_output_windows
        srcWin = used_windows#[i]
        if srcWin > 0
            # Color by source position (early=blue, late=red)
            colorVal = srcWin / num_windows
            dotColor$ = "{" + fixed$(0.3 + colorVal * 0.5, 2) + ", " + fixed$(0.3, 2) + ", " + fixed$(0.8 - colorVal * 0.5, 2) + "}"
            Paint circle (mm): dotColor$, i, srcWin, 0.5
        endif
    endfor
    ...
    # Find feature range (handle undefined values)
    minF1 = undefined
    maxF1 = undefined
    for i to num_windows
    ...
    for i to num_windows
        if feature1#[i] <> undefined
            Paint circle (mm): "{0.3, 0.5, 0.8}", i, feature1#[i], 0.4
        endif
    endfor
    ...
    Font size: 7
    Colour: "{0.4, 0.4, 0.4}"
    Text: 0.5, "centre", 0.5, "half", "Analysis: " + analysis_name$ + " | Variation: " + variation_name$ + " (" + fixed$(variation_amount, 2) + ") | Threshold: " + fixed$(similarity_threshold, 2)
...
appendInfoLine: ""
appendInfoLine: "=== Done ==="
appendInfoLine: "Output: ", sound_name$, "_LZ"
