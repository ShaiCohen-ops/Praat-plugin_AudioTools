...
for grain to total_grains
    ...
    if gDur > 0
        ...
        Extract part: source_pos, source_pos + gDur, "Hanning", 1, "no"
        grainExtract = selected("Sound")
        
        # Apply pitch shift if enabled and grain is long enough
        pitch_factor = 2^(pShift / 12)
        if enable_pitch_shifting and gDur >= 0.04 and abs(pitch_factor - 1) > 0.02
            selectObject: grainExtract
            nocheck Lengthen (overlap-add): 80, 600, pitch_factor
            if numberOfSelected("Sound") > 0
                grainShifted = selected("Sound")
                removeObject: grainExtract
                grainExtract = grainShifted
            endif
        endif
        ...
        # Mix into output
        grainID$ = string$(grainExtract)
        grainStart$ = fixed$(grain_start, 6)
        
        selectObject: outputSound
        Formula (part): grain_start, grain_end, 1, 1, "self + object(" + grainID$ + ", x - " + grainStart$ + ")"
