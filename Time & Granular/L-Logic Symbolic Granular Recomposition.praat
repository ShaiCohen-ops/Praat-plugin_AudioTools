...
# into one sustained phi region. phiBridge = max gap to absorb.
phiBridge = 0.150
nBridged = 0
for i from 1 to maxMerged
    bStart[i] = 0
...
if nMerged > 0
    ...
    for i from 2 to nMerged
        ...
        currLbl$ = mLabel$[i]
        gapDur = mStart[i] - bEnd[nBridged]
        if currLbl$ = "phi" and prevLbl$ = "phi" and gapDur <= phiBridge
            bEnd[nBridged] = mEnd[i]
        elsif prevLbl$ = "phi" and (currLbl$ = "psi" or currLbl$ = "theta") and gapDur <= phiBridge and i < nMerged
            nextLbl$ = mLabel$[i + 1]
            if nextLbl$ = "phi"
                bEnd[nBridged] = mEnd[i]
            else
