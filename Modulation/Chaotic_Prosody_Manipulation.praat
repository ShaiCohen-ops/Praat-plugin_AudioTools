...
#
# Changelog v0.2:
#   - Input check, modern syntax, presets, visualization, array bounds
#
# Changelog v0.3:
#   - Added an F0 safety clamp in the logistic branch (mirrors the OU branch):
#     a custom Logistic_depth > 1 could drive F0 <= 0 and crash PitchTier.
#   - Visualization polished to house style (title band font 14, grey {0.94}
#     summary panel, full-precision RGB).
#   - Replaced non-ASCII Greek (theta/sigma/rho/beta) in printed info and plot.
# ============================================================
...
if pitch_mode = 1
    ...
    appendInfoLine: "Pitch: OU process (theta=", oU_theta, ", sigma=", oU_sigma, ")"
endif
appendInfoLine: "AM: Lorenz (sigma=", lorenz_sigma, ", rho=", lorenz_rho, ", beta=", lorenz_beta, ")"
appendInfoLine: ""
...
if pitch_mode = 1
    ...
    for i from 1 to n_points
        ...
        f0 = f0_base * factor
        # Safety clamp (a custom depth > 1 could otherwise drive F0 <= 0)
        if f0 < 20
            f0 = 20
        endif
...
if draw_visualization
    ...
    Paint rectangle: "{0.94, 0.94, 0.94}", 0, duration, minP - pMargin, maxP + pMargin
    ...
    if lorenzIdx > 1
        ...
        Paint rectangle: "{0.94, 0.94, 0.94}", minLX - lMargin, maxLX + lMargin, minLY - lMargin, maxLY + lMargin
        ...
        Axes: -20, 20, -30, 30
        Paint rectangle: "{0.94, 0.94, 0.94}", -20, 20, -30, 30
    endif
    ...
    Paint rectangle: "{0.94, 0.94, 0.94}", 0, duration, 0, 1.1
    ...
    if pitch_mode = 1
        ...
        pitchText$ = "OU theta=" + fixed$(oU_theta, 1) + " sigma=" + fixed$(oU_sigma, 0)
    endif
    ...
    Text: 0.5, "centre", 0.5, "half", pitchText$ + "  |  Lorenz sigma=" + fixed$(lorenz_sigma, 0) + " rho=" + fixed$(lorenz_rho, 0) + " beta=" + fixed$(lorenz_beta, 2) + "  |  base F0 " + fixed$(f0_base, 0) + " Hz"
