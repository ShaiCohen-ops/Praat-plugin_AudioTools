...
# Version: 1.5.0 (2026) - Added chroma (12 pitch classes) feature
# License: MIT License
...
form Self-Similarity Matrix v1.5.0
    optionmenu Feature: 3
        ...
        option Chroma (12 pitch classes)
    comment === Analysis ===
...
# Chroma. A 25 ms window cannot resolve a semitone anywhere near the bottom of
# the musical range (a semitone at C3 is about 8 Hz wide), so chroma uses its
# own longer window and finer frequency grid. The range starts at C3 rather
# than at the pitch floor for the same reason.
chroma_window_length = 0.09
chroma_freq_step = 5
chroma_min_hz = 130.81
chroma_max_hz = 2093.0
chroma_ref_hz = 440
...
clearinfo
writeInfoLine: "=== Self-Similarity Matrix v1.5.0 ==="
...
if feature = 1
    ...
    # CHROMA: fold spectral magnitude onto the 12 pitch classes, so that two
    # frames match when they share a harmonic/pitch-class content regardless of
    # register or timbre. Complements MFCC, which is register-insensitive but
    # pitch-blind.
    selectObject: workingID
    sr = Get sampling frequency
    nyquist = sr / 2
    chroma_hi_hz = chroma_max_hz
    if chroma_hi_hz > nyquist
        chroma_hi_hz = nyquist
    endif
    if chroma_hi_hz <= chroma_min_hz * 2
        exitScript: "Sampling rate is too low for chroma: less than one octave above " + fixed$(chroma_min_hz, 1) + " Hz."
    endif

    specID = To Spectrogram: chroma_window_length, chroma_hi_hz, time_step, chroma_freq_step, "Gaussian"
    selectObject: specID
    matID = To Matrix
    selectObject: matID
    ny = Get number of rows
    ...
    # Map every spectrogram row to a semitone and a pitch class, once.
    # The frequency grid is linear, so a high semitone covers many more rows
    # than a low one. Folding raw row sums would therefore let the top octave
    # dominate the chroma vector; averaging within each semitone band first
    # gives every semitone the same weight before the octaves are summed.
    for m from 0 to 127
        semi_count_'m' = 0
    endfor
    chroma_row_lo = 0
    ...
    for j from 1 to ny
        freq = y1 + (j - 1) * dy
        if freq >= chroma_min_hz and freq <= chroma_hi_hz
            midi_'j' = round(69 + 12 * ln(freq / chroma_ref_hz) / ln(2))
            mj = midi_'j'
            if mj >= 0 and mj <= 127
                semi_count_'mj' = semi_count_'mj' + 1
                pc_'j' = mj mod 12 + 1
                if chroma_row_lo = 0
                    chroma_row_lo = j
                endif
    ...
    if chroma_row_lo = 0
        exitScript: "No usable frequency rows for chroma analysis."
    endif
    for j from chroma_row_lo to chroma_row_hi
        mj = midi_'j'
        cnt = semi_count_'mj'
        if cnt > 0
            wgt_'j' = 1 / cnt
    ...
    for i from 1 to nx
        if (i - 1) mod effective_skip = 0
            ...
            for j from chroma_row_lo to chroma_row_hi
                ...
                if val > 0
                    cj = pc_'j'
                    # Spectrogram cells hold power; chroma is conventionally
                    # built from magnitude, which is far less peaky.
                    chr_'cj' = chr_'cj' + sqrt(val) * wgt_'j'
                endif
            endfor

            selectObject: featureMatrixID
            for c from 1 to num_features
                Set value: out_f, c, chr_'c'
            endfor
            selectObject: matID
        endif
    endfor
...
# MFCC, LPC, Mel and Chroma use row-normalized vectors and cosine similarity.
if feature = 3 or feature = 5 or feature = 6 or feature = 7
...
if feature = 1
    ...
    if feature = 3 or feature = 5
        ...
        # Mel and chroma vectors are non-negative, hence cosine is already [0,1].
        Formula: "min(max(" + formula$ + ", 0), 1)"
    endif
endif
