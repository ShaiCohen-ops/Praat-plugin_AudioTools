...
#   - Added state trajectory visualization
#   - Added spatial modes
...
procedure drawVisualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.7, "half", "Markov Synthesis — " + preset_name$
    Font size: 10
    Colour: "{0.4, 0.4, 0.4}"
    Text: 0.5, "centre", 0.2, "half", markov_type$ + " | " + string$(number_of_states) + " states | " + string$(eventCount) + " events"
    ...
    Select inner viewport: .leftMargin, .rightMargin, 1.1, 2.7
    ...
    for ev to eventCount
        ...
        Paint circle: "{" + fixed$(.r, 2) + ", " + fixed$(.g, 2) + ", " + fixed$(.b, 2) + "}", eventTime[ev], .state, 0.08
    endfor
    ...
    # === Spectrogram ===
    Select outer viewport: 0, 7, 3.0, 5.3
    Colour: "{0.85, 0.85, 0.85}"
    Draw inner box
    
    Select inner viewport: .leftMargin, .rightMargin, 3.1, 5.2
    ...
    # === Footer ===
    Select outer viewport: 0, 7, 5.4, 5.8
    Select inner viewport: 0, 7, 5.4, 5.8
    Axes: 0, 1, 0, 1
    Font size: 8
    Colour: "{0.4, 0.4, 0.4}"
    .paramText$ = "Base: " + fixed$(base_frequency_Hz, 0) + " Hz | Density: " + fixed$(event_density, 1) + " | Randomness: " + fixed$(transition_randomness, 2)
