...
#
# Changelog v0.2:
#   - Uses all 3 dimensions for audio
#   - Added spatial modes
...
appendInfoLine: "Parameters: σ=", sigma, " ρ=", rho, " β=", fixed$(beta, 4)
...
if draw_visualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.7, "half", "Lorenz Attractor — " + preset_name$
    Font size: 10
    Colour: "{0.4, 0.4, 0.4}"
    Text: 0.5, "centre", 0.2, "half", "σ=" + fixed$(sigma, 1) + " ρ=" + fixed$(rho, 1) + " β=" + fixed$(beta, 3)
    ...
    Paint circle (mm): "{0.9, 0.2, 0.2}", finalX, finalZ, 2
...
if draw_visualization
    ...
    Font size: 8
    Colour: "{0.4, 0.4, 0.4}"
    Text: 0.5, "centre", 0.5, "half", "Left: Butterfly attractor (X vs Z) | Right: Audio spectrogram | x→pitch, y→amplitude, z→pan"
endif
