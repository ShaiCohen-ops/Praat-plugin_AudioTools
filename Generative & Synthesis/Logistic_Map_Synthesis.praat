...
#
# Changelog v0.2:
#   - Added fade in/out, spectrogram panel, fixed preset_name$
#
# Changelog v0.3:
...
#   - Replaced non-ASCII characters (multiplication sign, approx, em-dash).
# ============================================================
...
if draw_visualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.5, "half", "Logistic Map Synthesis: " + preset_name$
    ...
    if plot_type = 1 or plot_type = 3
        ...
        Paint circle (mm): "{0.20, 0.80, 0.20}", initial_x, 0, 2
    endif
        ...
        for i from startIdx to totalCtrlSamples - 1
            if (i mod 3) = 0
                ...
                if valX >= 0 and valX <= 1 and valY >= 0 and valY <= 1
                    Paint circle (mm): "{1.0, 0.50, 0.0}", valX, valY, 0.4
                endif
            endif
        endfor
    endif
...
if draw_visualization
    ...
    Text: 0.5, "centre", 0.5, "half", "x[n+1] = r * x[n] * (1 - x[n]) | r = " + fixed$(r_parameter, 4) + " | Base: " + fixed$(base_frequency_Hz, 0) + " Hz | Left: cobweb/return map, Right: spectrogram"
    Font size: 10
    Colour: "Black"
endif
