...
#
# Changelog v0.2:
#   - Focused on Layered Markov (removed redundant modes)
...
appendInfoLine: "Layer spacing: ", layer_spacing, "×"
...
procedure drawVisualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.7, "half", "Layered Markov Texture — " + preset_name$
    Font size: 10
    Colour: "{0.4, 0.4, 0.4}"
    Text: 0.5, "centre", 0.2, "half", string$(number_of_layers) + " layers × " + fixed$(events_per_layer, 1) + " events/layer | " + string$(totalEventCount) + " total events"
    
    # === Layer Activity Plot ===
    Select outer viewport: 0, 7, 1.0, 2.6
    Colour: "{0.95, 0.95, 0.95}"
    Paint rectangle: "{0.95, 0.95, 0.95}", 0, 1, 0, 1
    
    Select inner viewport: .leftMargin, .rightMargin, 1.1, 2.5
    ...
    for ev to totalEventCount
        ...
        # Draw event as horizontal bar
        .evStart = eventTime[ev]
        .evEnd = eventTime[ev] + eventDur[ev]
        .yPos = .lay
        Paint rectangle: "{" + fixed$(.r, 2) + ", " + fixed$(.g, 2) + ", " + fixed$(.b, 2) + "}", .evStart, .evEnd, .yPos - 0.3, .yPos + 0.3
    endfor
    ...
    # === Spectrogram ===
    Select outer viewport: 0, 7, 2.8, 5.1
    Colour: "{0.85, 0.85, 0.85}"
    Draw inner box
    
    Select inner viewport: .leftMargin, .rightMargin, 2.9, 5.0
    ...
    # === Footer ===
    Select outer viewport: 0, 7, 5.2, 5.6
    Select inner viewport: 0, 7, 5.2, 5.6
    Axes: 0, 1, 0, 1
    Font size: 8
    Colour: "{0.4, 0.4, 0.4}"
    .paramText$ = "Base: " + fixed$(base_frequency_Hz, 0) + " Hz | Spacing: " + fixed$(layer_spacing, 2) + "× | Complexity: " + fixed$(complexity, 2)
