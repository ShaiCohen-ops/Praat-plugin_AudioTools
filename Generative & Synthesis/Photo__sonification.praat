...
# Version: 0.4 (2025) - Fast envelope build (single-pass lookup)
# License: MIT License
...
#
# Changelog v0.4:
#   - Step envelopes built with one lookup Formula each (was one whole-buffer
#     Formula per column); ~20-100x faster envelope stage.
# ============================================================
...
# step envelope with a SINGLE nearest-column lookup Formula. This replaces
# rebuilding every envelope buffer once per column (was O(columns x duration)).
rVals = Create Sound from formula: "rVals_" + uid$, 1, 0, analysis_columns, 1, "0"
gVals = Create Sound from formula: "gVals_" + uid$, 1, 0, analysis_columns, 1, "0"
bVals = Create Sound from formula: "bVals_" + uid$, 1, 0, analysis_columns, 1, "0"
aVals = Create Sound from formula: "aVals_" + uid$, 1, 0, analysis_columns, 1, "0"
pVals = Create Sound from formula: "pVals_" + uid$, 1, 0, analysis_columns, 1, "0"

for aCol to analysis_columns
    selectObject: rVals
    Set value at sample number: 1, aCol, rNorm[aCol]
    selectObject: gVals
    Set value at sample number: 1, aCol, gNorm[aCol]
    selectObject: bVals
    Set value at sample number: 1, aCol, bNorm[aCol]
    selectObject: aVals
    Set value at sample number: 1, aCol, ampCol[aCol]
    selectObject: pVals
    Set value at sample number: 1, aCol, panCol[aCol]
endfor

rValsName$ = "Sound_rVals_" + uid$
gValsName$ = "Sound_gVals_" + uid$
bValsName$ = "Sound_bVals_" + uid$
aValsName$ = "Sound_aVals_" + uid$
pValsName$ = "Sound_pVals_" + uid$

redEnv   = Create Sound from formula: "redEnv_" + uid$,   1, 0, duration_s, sample_rate_Hz, rValsName$ + "[min(analysis_columns, floor(x / chunkDuration) + 1)]"
greenEnv = Create Sound from formula: "greenEnv_" + uid$, 1, 0, duration_s, sample_rate_Hz, gValsName$ + "[min(analysis_columns, floor(x / chunkDuration) + 1)]"
blueEnv  = Create Sound from formula: "blueEnv_" + uid$,  1, 0, duration_s, sample_rate_Hz, bValsName$ + "[min(analysis_columns, floor(x / chunkDuration) + 1)]"
ampEnv   = Create Sound from formula: "ampEnv_" + uid$,   1, 0, duration_s, sample_rate_Hz, aValsName$ + "[min(analysis_columns, floor(x / chunkDuration) + 1)]"
...
removeObject: rVals, gVals, bVals, aVals, pVals
