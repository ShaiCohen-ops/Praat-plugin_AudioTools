...
#
# Changelog v0.6.1:
#   - Restored the intended down-only peak-protection semantics.
#   - Added an explicit Peak_protection form control; output below 0.9 is no longer
#     raised by unconditional peak normalization.
#   - Updated Info / visualization wording from normalization to protection.
# ============================================================

form Poisson Point Process Synthesis v0.6.1
    optionmenu Preset 1
        option Custom
...
# ---------------------------------------------------------------------------
if edit_details
    beginPause: "Poisson Point Process v0.6.1 - Grain / Reproducibility Details"
...
# === Down-only peak protection / measured output QC ===
...
protectionApplied = 0
if nPointsLeft + nPointsRight > 0 and preProtectPeak > 0
    if peak_protection and preProtectPeak > 0.9
        Scale peak: 0.9
        protectionApplied = 1
    endif
else
...
appendInfoLine: "Output pre-protection peak/RMS: ", fixed$(preProtectPeak, 4), " / ", fixed$(preProtectRMS, 4)
...
procedure drawVisualization
    ...
    Font size: 8
    Colour: "Black"
    Text: 0.5,"centre",0.52,"half", "D  MEASURED OUTPUT | post-width, post-fade, post-protection waveform (confirmation, not the model)"
    ...
    if protectionApplied
        .protect$ = "down-only protection applied"
    else
        .protect$ = "level preserved"
    endif
    Text: 0.02,"left",0.06,"half", "OUTPUT QC | Poisson events L/R " + string$(nPointsLeft) + "/" + string$(nPointsRight) + " | final peak/RMS " + fixed$(finalPeak,3) + "/" + fixed$(finalRMS,4) + " | " + .protect$
