...
procedure drawVisualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.5, "half", "Poisson Rhythm: " + preset_name$

    # --- Panel 1: Event timeline (beats bottom, subdivisions top) ---
    Select outer viewport: 0, 8, 0.9, 2.5
    Select inner viewport: 0.75, 7.6, 1.05, 2.4
    Axes: 0, .dur, 0, 2
    Paint rectangle: "{0.95, 0.95, 0.95}", 0, .dur, 0, 2

    for .b to nBeats
        .t = beatTime[.b]
        Paint circle (mm): "{0.20, 0.40, 0.80}", .t, 0.5, 2
    endfor
    for .s to nSubs
        .t = subTime[.s]
        Paint circle (mm): "{0.80, 0.50, 0.20}", .t, 1.5, 1
    endfor
    ...
    # --- Summary panel (grey) ---
    if canon_mode > 1
        .summary$ = "Beats: " + string$(nBeats) + " (" + fixed$(beat_rate, 1) + "/s) | Subs: " + string$(nSubs) + " (" + fixed$(subdivision_rate, 1) + "/s) | Base: " + fixed$(base_frequency_Hz, 0) + " Hz | Canon: " + string$(canon_mode) + " voices | Blue=beats, Orange=subs"
    else
        .summary$ = "Beats: " + string$(nBeats) + " (" + fixed$(beat_rate, 1) + "/s) | Subs: " + string$(nSubs) + " (" + fixed$(subdivision_rate, 1) + "/s) | Base: " + fixed$(base_frequency_Hz, 0) + " Hz | Blue=beats, Orange=subs"
    endif
