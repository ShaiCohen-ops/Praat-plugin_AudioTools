...
procedure makeSynth: .freq, .dur
    ...
    .fadeout$ = "self * if x > " + fixed$(.tdur - 0.01, 6)
    .fadeout$ = .fadeout$ + " then (" + fixed$(.tdur, 6) + "-x)/0.01 else 1 fi"
    Formula: .fadeout$
    Scale peak: 0.9
...
procedure drawVisualization
  ...
  # === Title ===
    ...
    # Force a strict 0 to 1 coordinate space to prevent squishing
