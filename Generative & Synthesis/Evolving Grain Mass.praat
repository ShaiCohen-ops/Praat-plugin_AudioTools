...
#
# Changelog v0.2:
#   - Chunked grain processing (no string explosion)
...
procedure drawVisualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.8, "half", "Evolving Grain Mass — " + preset_name$
    
    Font size: 10
    Colour: "{0.4, 0.4, 0.4}"
    Text: 0.5, "centre", 0.2, "half", evolution_type$
    ...
    # === Footer (with explicit positioning) ===
    ...
    Font size: 8
    Colour: "{0.4, 0.4, 0.4}"
    .paramText$ = "Density: " + fixed$(initial_density, 0) + "→" + fixed$(final_density, 0) + " | Base: " + fixed$(base_frequency_Hz, 0) + " Hz | Grains: " + string$(totalGrains)
