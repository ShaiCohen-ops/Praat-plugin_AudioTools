...
#
# Changelog v0.4:
#   - Corrected the description (was mislabelled "Accelerating Polyrhythm
#     Generator"; the script is a timbre-mutation generator).
#   - Rebuilt the visualization in the AudioTools house style (title band, grey
#     {0.94} summary, 8-inch canvas, full-precision RGB) while keeping the dark
#     score. Fixed the info legend, which was drawn in world coordinates and
#     landed off in the corner; it now sits in the summary panel.
#   - Replaced non-ASCII characters (arrows, en-dash).
#
# Description:
#   Risset-inspired timbre-mutation generator. Scheduled sine events are morphed
#   through waveshaping spectra (odd-harmonic "woody", brassy, and high-order
#   "glassy" Chebyshev shapes) that swell in and out mid-event, so each tone
#   mutates in timbre over its duration.
#
# Usage:
#   RISSET'S MUTATIONS
#
...
if show_visuals
    ...
    Colour: "Black"
    if preset = 1
        Text: 0.5, "centre", 0.5, "half", "Risset's Mutations: Full Composition (3-Part Arc)"
    elsif preset = 2
        Text: 0.5, "centre", 0.5, "half", "Risset's Mutations: Study in Odd Harmonics (Woody/Hollow)"
    elsif preset = 3
        Text: 0.5, "centre", 0.5, "half", "Risset's Mutations: Study in High Order (Glassy/Chime)"
    else
        Text: 0.5, "centre", 0.5, "half", "Risset's Mutations: Study in Density (Brassy/Chaos)"
    endif

    # === Score area (dark spectral plot) ===
    Select outer viewport: 0, 8, 0.9, 5.4
    Select inner viewport: 0.8, 7.6, 1.05, 5.3
    ...
    # Events with time-based colour gradient (blue -> cyan -> green -> yellow -> red)
    Line width: 2
    for i from 1 to n_events
        t1 = start_t[i]
        d = dur[i]
    ...
    Text: 0.5, "centre", 0.5, "half", "Events: " + string$(n_events) + " | Duration: " + fixed$(master_duration, 1) + " s | Base: " + fixed$(base_freq, 0) + " Hz | Colour: blue -> red (time)"
    Font size: 10
    Colour: "Black"
    Line width: 1
endif
