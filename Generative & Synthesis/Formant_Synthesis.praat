...
#
# Changelog v0.2:
#   - Added spatial modes, visualization, morphing vowel option
#
# Changelog v0.3:
#   - Rebuilt the visualization in the AudioTools house style (8-inch canvas,
#     inner-viewport margins, font-12 title, grey summary panel). Content kept:
#     F1/F2 vowel chart with the current vowel, and the output spectrogram
#     with F1-F4 formant lines.
#   - Replaced non-ASCII glyphs (bullet, arrows, dashes) with ASCII.
# ============================================================
...
procedure drawVisualization
    ...
    # --- Title (its own clear band at the very top) ---
    Select outer viewport: 0, 8, 0.1, 0.7
    Axes: 0, 1, 0, 1
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.5, "half", "Formant Synthesis (KlattGrid): " + preset_name$

    # ============ LEFT: Vowel chart (F1 vs F2) ============
    Select outer viewport: 0, 4, 1.0, 4.4
    Select inner viewport: 0.95, 3.6, 1.15, 4.2
    .f2Min = 500
    .f2Max = 2500
    .f1Min = 200
    .f1Max = 900
    Axes: .f2Max, .f2Min, .f1Max, .f1Min
    Paint rectangle: "{0.95, 0.95, 0.95}", .f2Min, .f2Max, .f1Min, .f1Max
    ...
    Colour: "{0.50, 0.50, 0.80}"
    Paint circle: "{0.50, 0.50, 0.80}", 2290, 270, 40
    Paint circle: "{0.50, 0.50, 0.80}", 1840, 530, 40
    Paint circle: "{0.50, 0.50, 0.80}", 1090, 730, 40
    Paint circle: "{0.50, 0.50, 0.80}", 840, 570, 40
    Paint circle: "{0.50, 0.50, 0.80}", 870, 300, 40
    Colour: "Black"
    Font size: 13
    ...
    Paint circle: "{0.90, 0.20, 0.20}", f2, f1, 60
    Colour: "White"
    Font size: 11
    Text: f2, "centre", f1, "half", "o"

    Colour: "Black"
    Draw inner box
    Font size: 10
    Text bottom: "yes", "F2 (Hz): front (left) to back (right)"
    Text left: "yes", "F1 (Hz): close (top) to open (bottom)"

    # ============ RIGHT: Spectrogram + formant lines ============
    Select outer viewport: 4, 8, 1.0, 4.4
    Select inner viewport: 4.8, 7.7, 1.15, 4.2
    ...
    Line width: 2
    Colour: "{1.0, 0.40, 0.40}"
    Draw line: 0, f1, duration_s, f1
    Colour: "{0.40, 1.0, 0.40}"
    Draw line: 0, f2, duration_s, f2
    Colour: "{0.40, 0.40, 1.0}"
    ...
    Text: 0.5, "centre", 0.5, "half", "F1=" + fixed$(f1, 0) + " F2=" + fixed$(f2, 0) + " F3=" + fixed$(f3, 0) + " F4=" + fixed$(f4, 0) + " Hz | BW: " + fixed$(bW1, 0) + "/" + fixed$(bW2, 0) + "/" + fixed$(bW3, 0) + "/" + fixed$(bW4, 0) + " Hz | Pitch: " + fixed$(pitch_Hz, 0) + " Hz | Red dot = current vowel"
    Font size: 10
    Colour: "Black"
    Line width: 1
endproc
