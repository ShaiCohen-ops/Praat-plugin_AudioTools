...
#     title band, vowel-colored grain timeline + legend + spectrogram, grey
#     summary, larger fonts, full-precision RGB).
#   - Replaced non-ASCII characters (IPA epsilon in a comment, em-dash).
# ============================================================
...
procedure drawVisualization
    ...
    # --- Panel 1: Grain timeline (colored by vowel, F1-F2 band) ---
    Select outer viewport: 0, 8, 0.9, 2.5
    Select inner viewport: 0.75, 7.6, 1.05, 2.4
    Axes: 0, duration_s, 0, 4000
    Paint rectangle: "{0.95, 0.95, 0.95}", 0, duration_s, 0, 4000
    ...
    # --- Panel 2: Vowel legend ---
    Select outer viewport: 0, 8, 2.6, 3.0
    Select inner viewport: 0.75, 7.6, 2.65, 2.95
    Axes: 0, 5, 0, 1
    Paint rectangle: "{0.90, 0.30, 0.30}", 0, 0.8, 0.2, 0.8
    Paint rectangle: "{0.30, 0.90, 0.30}", 1, 1.8, 0.2, 0.8
    Paint rectangle: "{0.30, 0.30, 0.90}", 2, 2.8, 0.2, 0.8
    Paint rectangle: "{0.90, 0.60, 0.20}", 3, 3.8, 0.2, 0.8
    Paint rectangle: "{0.60, 0.30, 0.90}", 4, 4.8, 0.2, 0.8
    Font size: 9
    Colour: "Black"
    ...
    Select inner viewport: 0.75, 7.6, 3.35, 5.1
    selectObject: outputSound
    ...
    Text: 0.5, "centre", 0.5, "half", string$(totalGrains) + " grains | Base: " + fixed$(base_frequency_Hz, 0) + " Hz | " + spatial_name$ + " | F0 + F1 + F2 + F3, half-sine envelope"
    Font size: 10
    Colour: "Black"
    Line width: 1
endproc