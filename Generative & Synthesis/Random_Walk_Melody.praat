...
#
# Changelog v0.2:
#   - Chunked synthesis, presets, scale options, visualization
#
# Changelog v0.3:
#   - Fixed a latent crash: Just Intonation and Equal Temperament did not clamp
#     scale_degrees to their ratio-table sizes (8 and 12), so a custom
#     scale_degrees above that (e.g. ET with 16) let the random walk index an
#     undefined ratio and abort. Now every tuning clamps to its table, plus a
#     lower bound of 2.
#   - Rebuilt the visualization in the AudioTools house style (8-inch canvas,
#     title band, pitch + frequency contour + spectrogram, grey summary, larger fonts).
...
# Select tuning (clamp scale_degrees to each tuning's ratio-table size)
if tuning = 1
...
procedure drawVisualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.5, "half", "Random Walk Melody: " + preset_name$

    # --- Panel 1: Pitch contour (scale degrees) ---
    Select outer viewport: 0, 8, 0.9, 2.6
    Select inner viewport: 0.75, 7.6, 1.05, 2.5
    Axes: 0, duration_s, 0, scale_degrees + 1
    Paint rectangle: "{0.95, 0.95, 0.95}", 0, duration_s, 0, scale_degrees + 1
    ...
    Line width: 1
    for .n to totalNotes
        Paint circle (mm): "{0.20, 0.50, 0.80}", noteTime[.n], noteDegree[.n], 1.5
    endfor
    ...
    # --- Panel 2: Frequency contour ---
    Select outer viewport: 0, 8, 2.8, 4.3
    Select inner viewport: 0.75, 7.6, 2.95, 4.2
    .minF = noteFreq[1]
    .maxF = noteFreq[1]
    for .n from 2 to totalNotes
        if noteFreq[.n] < .minF
    ...
    # --- Panel 3: Spectrogram ---
    Select outer viewport: 0, 8, 4.5, 6.0
    Select inner viewport: 0.75, 7.6, 4.65, 5.9
    selectObject: outputSound
    Copy: "temp_spec"
    ...
    Text: 0.5, "centre", 0.5, "half", tuning_name$ + " | " + string$(totalNotes) + " notes | " + string$(scale_degrees) + " degrees | Step prob: " + fixed$(step_probability, 2) + " | Max step: +/-" + string$(max_step_size) + " | Base: " + fixed$(base_frequency_Hz, 0) + " Hz"
    Font size: 10
    Colour: "Black"
    Line width: 1
endproc