...
#
# Changelog v0.3:
#   - Clave-like percussion (woody, sharp attack, inharmonic overtones)
#
# Changelog v0.4:
#   - Rebuilt the visualization in the AudioTools house style (8-inch canvas,
#     title in its own band, three Toussaint necklaces across the top,
#     spectrogram, state timeline, grey summary, larger fonts, full-precision RGB).
...
procedure drawVisualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.5, "half", "Markov Clave Generator: " + preset_name$

    # ===================== Toussaint necklaces (3 across) =====================
    # --- Circle 1 ---
    Select outer viewport: 0, 2.67, 0.8, 3.0
    Select inner viewport: 0.4, 2.3, 0.95, 2.85
    ...
    for .i to .len1
        ...
        if .char$ = "1"
            ...
            Paint circle: "{0.60, 0.30, 0.10}", .px, .py, 0.12
        else
            Paint circle: "{0.80, 0.80, 0.80}", .px, .py, 0.05
        endif
    endfor
    if .onsetCount1 > 1
        ...
        for .k to .onsetCount1
            ...
            Draw line: cos(.onsetAngle1[.k]) * 0.7, sin(.onsetAngle1[.k]) * 0.7, cos(.onsetAngle1[.nextK]) * 0.7, sin(.onsetAngle1[.nextK]) * 0.7
        endfor
        Line width: 1
    endif
    Colour: "Black"
    ...
    # --- Circle 2 ---
    Select outer viewport: 2.67, 5.33, 0.8, 3.0
    Select inner viewport: 3.05, 4.95, 0.95, 2.85
    Axes: -1.4, 1.4, -1.4, 1.4
    ...
    for .i to .len2
        ...
        if .char$ = "1"
            ...
            Paint circle: "{0.50, 0.25, 0.10}", .px, .py, 0.12
        else
            Paint circle: "{0.80, 0.80, 0.80}", .px, .py, 0.05
        endif
    endfor
    if .onsetCount2 > 1
        Colour: "{0.70, 0.40, 0.15}"
        Line width: 2
        for .k to .onsetCount2
            ...
            Draw line: cos(.onsetAngle2[.k]) * 0.7, sin(.onsetAngle2[.k]) * 0.7, cos(.onsetAngle2[.nextK]) * 0.7, sin(.onsetAngle2[.nextK]) * 0.7
        endfor
        Line width: 1
    endif
    Colour: "Black"
    ...
    # --- Circle 3 ---
    Select outer viewport: 5.33, 8, 0.8, 3.0
    Select inner viewport: 5.7, 7.6, 0.95, 2.85
    Axes: -1.4, 1.4, -1.4, 1.4
    ...
    for .i to .len3
        ...
        if .char$ = "1"
            ...
            Paint circle: "{0.40, 0.20, 0.05}", .px, .py, 0.12
        else
            Paint circle: "{0.80, 0.80, 0.80}", .px, .py, 0.05
        endif
    endfor
    if .onsetCount3 > 1
        Colour: "{0.60, 0.35, 0.10}"
        Line width: 2
        for .k to .onsetCount3
            ...
            Draw line: cos(.onsetAngle3[.k]) * 0.7, sin(.onsetAngle3[.k]) * 0.7, cos(.onsetAngle3[.nextK]) * 0.7, sin(.onsetAngle3[.nextK]) * 0.7
        endfor
        Line width: 1
    endif
    Colour: "Black"
    ...
    # ===================== Spectrogram =====================
    Select outer viewport: 0, 8, 3.2, 4.9
    Select inner viewport: 0.75, 7.6, 3.35, 4.8
    if canonVoices > 1
        selectObject: outputSound
    ...
    Select inner viewport: 0.75, 7.6, 5.25, 5.8
    .showBeats = min(16, totalBeats)
    Axes: 0, .showBeats, 0, rhythmStates + 1
    Paint rectangle: "{0.95, 0.95, 0.95}", 0, .showBeats, 0, rhythmStates + 1
    ...
    for .p to totalPulses
        if pulseBeat[.p] <= .showBeats
            ...
            .voice = pulseVoice[.p]
            if .voice = 1
                Paint circle: "{0.60, 0.30, 0.10}", .beat - 0.5, .state, 0.15
            elsif .voice = 2
                Paint circle: "{0.50, 0.25, 0.10}", .beat - 0.5, .state, 0.15
            else
                Paint circle: "{0.40, 0.20, 0.05}", .beat - 0.5, .state, 0.15
            endif
        endif
    endfor
    ...
    Marks left: rhythmStates, "yes", "yes", "no"
    Font size: 10
    Text bottom: "yes", "Beat"
    Text left: "yes", "State"

    # ===================== Summary panel (grey) =====================
    Select outer viewport: 0, 8, 6.0, 6.4
    Axes: 0, 1, 0, 1
    ...
    Text: 0.5, "centre", 0.5, "half", string$(tempo_bpm) + " BPM | " + fixed$(base_frequency_Hz, 0) + " Hz | " + string$(totalPulses) + " hits | Wood ratio: " + fixed$(woodRatio, 2) + " | Decay: " + fixed$(decay_rate, 0) + " | Toussaint necklaces"
    Font size: 10
    Colour: "Black"
    Line width: 1
endproc
