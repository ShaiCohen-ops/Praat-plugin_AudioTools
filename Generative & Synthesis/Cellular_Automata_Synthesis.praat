...
#
# Changelog v0.3:
#   - Fix 1: 2D CA footer viewport y-coords were inverted (7.4,7) —
#     y1 > y2 so nothing drew; corrected to (4.6,5.2).
#   - Fix 2: 1D CA synthesis had no formula-size cap; dense states
#     (Rule 90 ~50% density, 64 cells) built ~7 KB strings per segment
#     causing hangs on longer runs. Added 50-cell flush/reset identical
#     to the 2D path.
#   - Fix 3: ca_history_count[] was filled each segment but never read
#     or drawn anywhere; removed dead storage block.
#   - Fix 4: Spectrogram max-freq formula was min(4000,...) which was
#     already content-adaptive and correct. Added a 500 Hz floor via
#     min(4000, max(500,...)) so presets with very small base+spread
#     still get a usable display range.
#   - Fix 5: Added Axes: 0,1,0,1 before footer Text top (set axes
#     explicitly before any viewport text/paint per gotcha library).
# ============================================================
...
if rule_type = 1
    ...
    for seg to totalSegments
        ...
        if activeCells > 0
            # Build formula for this segment — cap at 50 cells per flush
            # (mirrors the 2D path) to prevent formula-string blow-up on
            # dense states (Rule 90 ~50% density, 64 cells ≈ 7 KB/string).
            segFormula$ = "0"
            cellCount1D = 0
            ...
            for cell to grid_size
                if ca_state[seg, cell] = 1
                    ...
                    cellCount1D = cellCount1D + 1
                    
                    # Flush at 50 cells to cap formula-string length
                    if cellCount1D >= 50
...
procedure drawCAVisualization
    ...
    # === Title ===
    Select outer viewport: 0, 7, 0, 0.65
    Font size: 12
    Colour: "Black"
    Axes: 0, 1, 0, 1
    if rule_type = 1
        Text: 0.5, "centre", 0.5, "half", "Cellular Automata: Rule " + string$(rule_number)
    elsif rule_type = 2
        Text: 0.5, "centre", 0.5, "half", "Cellular Automata: Game of Life"
    else
        Text: 0.5, "centre", 0.5, "half", "Cellular Automata: Brian's Brain"
    endif
    
    # === CA Grid (for Elementary CA) ===
    if rule_type = 1
        Select outer viewport: 0, 7, 0.7, 3.0
        Colour: "{0.9, 0.9, 0.9}"
        Draw inner box
        
        Select inner viewport: .leftMargin, .rightMargin, 0.8, 2.9
        ...
        Select outer viewport: 0, .leftMargin, 0.7, 3.0
        Colour: "Black"
        Font size: 9
    ...
    if rule_type = 1
        ...
        Select outer viewport: 0, 7, 0.7, 4.5
    endif
    ...
    if rule_type = 1
        ...
        Select inner viewport: .leftMargin, .rightMargin, 0.8, 4.4
    endif
    ...
    .maxFreqSpec = min(4000, max(500, base_frequency_Hz + frequency_spread_Hz * 1.5))
    ...
    if rule_type = 1
        ...
        Select outer viewport: 0, .leftMargin, 0.7, 4.5
    endif
    Colour: "Black"
    Font size: 9
    ...
    if rule_type = 1
        ...
        Select outer viewport: 0, 7, 4.6, 5.2
    endif
    Axes: 0, 1, 0, 1
    Font size: 8
    Colour: "{0.4, 0.4, 0.4}"
