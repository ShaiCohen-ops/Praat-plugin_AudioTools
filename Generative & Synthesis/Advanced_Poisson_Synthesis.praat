...
#
# Changelog v0.2:
#   - Fixed filters, fade if/fi, chunked grains, sample rate, viz
#
# Changelog v0.3:
#   - Fixed preset mode labels: presets set the numeric synthesis_mode but left
#     synthesis_mode$ at the form default, so every preset reported "Three Layer
#     Standard" in the info window and plot title regardless of the actual mode.
...
if preset = 2
    ...
    synthesis_mode = 1
    spatial_mode = 1
    synthesis_mode$ = "Three Layer Standard"
    ...
    synthesis_mode = 4
    spatial_mode = 1
    synthesis_mode$ = "Rhythmic Pulses"
    ...
    synthesis_mode = 5
    spatial_mode = 2
    synthesis_mode$ = "Chaotic Scatter"
    preset_name$ = "Chaotic"
elsif preset = 7
    duration_s = 10
...
procedure drawSpectrogram: .duration
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.5, "half", "Poisson Synthesis: " + preset_name$ + " (" + synthesis_mode$ + ")"
    ...
    Text: 0.5, "centre", 0.5, "half", "Rate: " + fixed$(low_rate_Hz, 1) + "-" + fixed$(high_rate_Hz, 1) + " Hz | Grains: " + string$(totalGrains) + " | Layers: " + string$(number_of_layers) + " | Base: " + fixed$(base_frequency_Hz, 0) + " Hz | " + .spatial$
    Font size: 10
    Colour: "Black"
    Line width: 1
endproc