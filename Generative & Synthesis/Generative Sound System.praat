...
#   - Fixed nested if/fi in envelope
#   - Fixed granular cloud mode
#   - Added visualization
...
if spatial_mode = 2
    ...
    # Slight delay and filtering for width
    Formula: "if col > 50 then self[col - 50] * 1.05 else self fi"
    ...
    delaySamples = 100
    Formula: "if col > " + string$(delaySamples) + " then self[col - " + string$(delaySamples) + "] else 0 fi"
...
procedure drawVisualization
    ...
    Font size: 14
    Colour: "Black"
    Text: 0.5, "centre", 0.8, "half", "Generative Sound System — " + mode_name$
    Font size: 10
    Colour: "{0.4, 0.4, 0.4}"
    Text: 0.5, "centre", 0.2, "half", string$(number_of_layers) + " layers | Evolution: " + fixed$(evolution_rate, 2)
    ...
    Font size: 8
    Colour: "{0.4, 0.4, 0.4}"
    .paramText$ = "Base: " + fixed$(base_frequency_Hz, 0) + " Hz | Fade: " + fixed$(fade_time_s, 1) + " s | " + spatial_mode$
