...
#
# Changelog v2.1:
#   - Fixed preset labels: presets set the numeric waveform / filter_type /
#     filter_envelope / amplitude_envelope but left the companion $ strings at
#     the form default, so the info window, plot titles, and footer reported
#     "Sawtooth / Low Pass 12dB / Sustained" for every preset regardless of the
#     actual synthesis. Each preset now sets all four strings to match.
#   - Visualization aligned to the AudioTools house style (8-inch canvas, full-
#     precision RGB, grey {0.94} summary, larger fonts). The six-stage layout
#     (oscillator / filtered / final / spectrum / envelope / params) is kept.
...
procedure drawVisualization
    ...
    # === Panel 1: Raw oscillator ===
    Select outer viewport: 0, 4, 0.7, 2.0
    Select inner viewport: 0.55, 3.75, 0.85, 1.9
    ...
    # === Panel 2: After filter ===
    if filter_type > 1
        if filteredID > 0
            Select outer viewport: 4, 8, 0.7, 2.0
            Select inner viewport: 4.45, 7.65, 0.85, 1.9
    ...
    Select outer viewport: 0, 8, 2.2, 3.5
    Select inner viewport: 0.55, 7.65, 2.35, 3.4
    ...
    # === Panel 5: Amplitude envelope shape ===
    Select outer viewport: 4, 8, 3.7, 5.4
    Select inner viewport: 4.45, 7.65, 3.85, 5.3
    envDur = min(2, duration_s)
    Axes: 0, envDur, 0, 1.1
    Paint rectangle: "{0.94, 0.94, 0.94}", 0, envDur, 0, 1.1
    Colour: "{0.60, 0.30, 0.60}"
    Line width: 2
    numPoints = 200
    ...
    Select inner viewport: 0.55, 7.65, 5.65, 5.95
    Axes: 0, 1, 0, 1
