...
form Recording and Composition Settings
...
# 3. Trim Silence (Using your specific extended syntax)
...
# IMPROVED CLEANUP: Remove artifacts from Crystalline Cascade
nocheck removeObject: "Recording_cascade_Subtle"
nocheck removeObject: initial_name$ + "_cascade_Subtle"
nocheck removeObject: initial_name$ + "_Part3_Raw_cascade_Subtle"

# Comprehensive cleanup - find and remove any remaining cascade artifacts
appendInfoLine: "  Searching for cascade artifacts..."
# Get list of all Sound objects
select all
if numberOfSelected("Sound") > 0
    ...
    # Now remove cascade artifacts
    for j to n
        tempName$ = name'j'$
        # Check if name contains "cascade" and it's not our final sound or initial sound
        if index(tempName$, "cascade") > 0 and tempName$ <> final_name$ and tempName$ <> initial_name$
