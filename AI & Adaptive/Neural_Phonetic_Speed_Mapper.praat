...
#
# Changelog v0.4:
#   - VISUALIZATION STANDARDIZATION ONLY; feature extraction, FFNet
#     classification, probabilistic phonetic weights, duration-tier
#     construction and overlap-add resynthesis are unchanged.
...
#   - Preserved the original four visual elements: source waveform,
#     output waveform, phonetic-class weights and smoothed stretch curve.
#   - Added draw-safe source names and a measured summary of class balance,
#     stretch factors, formant evidence and output duration ratio.
#
# Changelog v0.3:
...
form Neural Phonetic Speed Mapper v0.4
    comment === Preset ===
    optionmenu Preset: 1
        option Manual
...
clearinfo
writeInfoLine: "=== Neural Phonetic Speed Mapper v0.4 ==="
...
if draw_visualization
    ...
    Font size: 12
    Colour: "Black"
    Text: 0.5, "centre", 0.68, "half", "##Neural Phonetic Speed Mapper v0.4##"
    Font size: 7
    Colour: "{0.35, 0.35, 0.50}"
    Text: 0.5, "centre", 0.22, "half", vizSoundName$ + " | " + presetName$ + " | " + string$(nFrames) + " analysis frames | temperature " + fixed$(temperature, 2)
    ...
    Text top: "no", "Speed-mapped Output | " + fixed$(out_dur, 2) + " s | ratio " + fixed$(out_dur / duration, 2) + "x"

    # === Phonetic class weights ===
    Select outer viewport: 0, 8, 2.64, 4.18
    Select inner viewport: 0.60, 7.70, 2.84, 3.96
    Axes: tmin, tmax, 0, 1
    ...
    Text bottom: "no", "Time (s)"
    Text top: "no", "Phonetic Weights | vowel red | consonant blue | other green | silence grey"

    # === Smoothed stretch curve ===
    Select outer viewport: 0, 8, 4.38, 5.70
    Select inner viewport: 0.60, 7.70, 4.58, 5.48
    ...
    Text bottom: "no", "Time (s)"
    Text top: "no", "Smoothed Duration Mapping | 1.0 = unchanged"

    # === Summary strip ===
    Select outer viewport: 0, 8, 5.90, 6.40
    Select inner viewport: 0.60, 7.70, 5.96, 6.34
    Axes: 0, 1, 0, 1
    Paint rectangle: "{0.94, 0.94, 0.94}", 0, 1, 0, 1

    Font size: 6
    Colour: "{0.25, 0.25, 0.35}"
    summary1$ = "##Classes##  vowel " + fixed$(100 * nVowelFr / nFrames, 1) + "\% | consonant " + fixed$(100 * nConsFr / nFrames, 1) + "\% | other " + fixed$(100 * nOtherFr / nFrames, 1) + "\% | silence " + fixed$(100 * nSilFr / nFrames, 1) + "\% "
    summary2$ = "##Mapping##  V " + fixed$(vowel_stretch, 2) + "x | C " + fixed$(consonant_stretch, 2) + "x | O " + fixed$(other_stretch, 2) + "x | S " + fixed$(silence_stretch, 2) + "x | smoothing " + fixed$(smoothing_ms, 0) + " ms"
    summary3$ = "##Evidence & output##  formant evidence " + fixed$(100 * validFraction, 1) + "\% (" + formantState$ + ") | " + fixed$(duration, 2) + " -> " + fixed$(out_dur, 2) + " s | ratio " + fixed$(out_dur / duration, 2) + "x"
