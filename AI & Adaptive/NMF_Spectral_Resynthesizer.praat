...
#     role assignment, H processing, additive resynthesis, pitch anchoring
#     and stereo rendering are unchanged from v0.6.
...
#   - Preserved the original visual structure: source/output waveforms,
#     W spectral bases and H activation trajectories.
#   - Added draw-safe source names and a measured/model summary including
#     reconstruction error, transient-component count and output mode.
#
# Changelog v0.4:
...
if draw_visualization
    ...
    vizName$ = replace$(name_original$, "_", "\_ ", 0)

    if pitch_anchoring = 1
        pitchDesc$ = "raw NMF resynthesis"
    elsif pitch_anchoring = 2
        pitchDesc$ = "original-contour PSOLA anchoring"
    else
        pitchDesc$ = "raw / anchored blend"
    endif

    if stereo_output
        stereoDesc$ = "stereo H decorrelation " + fixed$(stereo_decorrelation, 2)
    else
        stereoDesc$ = "mono output"
    endif

    selectObject: soundFinal
    finalPeakDisplay = Get absolute extremum: 0, 0, "None"
    finalDurDisplay = Get total duration
    ...
    # === Original waveform ===
    Select outer viewport: 0, 8, 0.66, 1.47
    Select inner viewport: 0.60, 7.70, 0.78, 1.31
    selectObject: id_original
    ...
    # === Output waveform ===
    Select outer viewport: 0, 8, 1.60, 2.41
    Select inner viewport: 0.60, 7.70, 1.72, 2.25
    selectObject: soundFinal
    ...
    Text top: "no", "NMF Resynthesis | " + string$(finalChannels) + " ch | peak " + fixed$(finalPeakDisplay, 3)

    # === W matrix: spectral basis vectors ===
    Select outer viewport: 0, 4, 2.61, 4.50
    Select inner viewport: 0.60, 3.85, 2.85, 4.25
    ...
    Paint rectangle: "{0.97, 0.97, 0.97}", 0, wCols, 0, wRows
    ...
    while r <= wRows
        for c from 1 to wCols
            ...
            rVal$ = fixed$(1 - intensity * 0.8, 3)
            gVal$ = fixed$(1 - intensity * 0.5, 3)
            bVal$ = fixed$(1 - intensity * 0.2, 3)
    ...
    Draw inner box
    Font size: 7
    Text left: "yes", "Frequency bin"
    Text bottom: "no", "Component"
    Text top: "no", "Spectral Bases W | deeper blue = stronger basis weight"

    # === H matrix: temporal activations ===
    ...
    # Draw each component as a line over the spectrogram's actual time grid.
    for comp from 1 to hRows
        if hRows > 1
            colorVal = (comp - 1) / (hRows - 1)
        else
            colorVal = 0
        endif
    ...
    Text bottom: "no", "Time (s)"
    Text top: "no", "Temporal Activations H | processed H for left / mono output"

    # === Summary strip ===
    Select outer viewport: 0, 8, 4.70, 5.70
    Select inner viewport: 0.60, 7.70, 4.78, 5.62
    Axes: 0, 1, 0, 1
    Paint rectangle: "{0.94, 0.94, 0.94}", 0, 1, 0, 1

    Font size: 6
    Colour: "{0.25, 0.25, 0.35}"
    summary1$ = "##Analysis##  window " + fixed$(window_ms, 1) + " ms | step " + fixed$(step_ms, 1) + " ms | max " + string$(max_freq_hz) + " Hz | " + string$(n_components) + " components"
    summary2$ = "##Factorization##  " + string$(n_iterations) + " iterations | relative reconstruction error " + fixed$(reconErrRel, 5) + " | transient components " + string$(nTransient) + " | smear passes " + string$(texture_smear_passes)
    summary3$ = "##Output##  " + pitchDesc$ + " | " + stereoDesc$ + " | " + fixed$(finalDurDisplay, 2) + " s | phase regenerated by additive resynthesis"
