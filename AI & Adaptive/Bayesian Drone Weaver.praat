...
#   - Restructured to avoid goto
#   - Added visualization
# ============================================================

form Bayesian Drone Weaver v0.2
...
clearinfo
writeInfoLine: "=== Bayesian Drone Weaver v0.2 ==="
...
procedure assembleDrone
    ...
    for .seg to timeline_n_segments
        ...
        # Mix into final sound using Object_ reference
        .start_sample = round(.start_time * target_sr)
        .end_sample = round((.start_time + .duration) * target_sr)
        .partIdStr$ = string$(.part)
        .startStr$ = string$(.start_sample)
        .endStr$ = string$(.end_sample)
        
        selectObject: final_sound
        Formula: "self + if col >= " + .startStr$ + " and col <= " + .endStr$ + " then Object_" + .partIdStr$ + "[col - " + .startStr$ + " + 1] else 0 endif"
