...
form SuperVP Transform v2.0
    ...
    comment === AGE ===
    integer Age_years 20

    comment === GENDER ===
    optionmenu Gender_direction: 1
        option To female
        option To male
    integer Gender_amount 3

    comment === FLATTEN PITCH ===
    real Target_f0_hz 0
    ...
    comment === TREMOLO ===
    positive Tremolo_freq_hz 5.0
    positive Tremolo_depth_cents 30

    comment === CROSS SYNTH ===
    real Cross_mix 0.5

    comment === VIBRATO ===
    positive Vibrato_freq_hz 5.5
    positive Vibrato_depth_cents 50

    comment === BREATHINESS ===
    real Breathiness_amount 0.4

    comment === FORMANT SHIFT ===
    integer Formant_shift_cents 200

    comment === HARMONISER ===
    real Harmoniser_interval_cents 700
    real Harmoniser_mix 0.3

    comment === DE-NOISE ===
    positive Denoise_threshold_db 30
...
if mode$ = "cross" and nSounds < 2
    exitScript: "Cross synth requires 2 selected Sound objects."
endif
...
# ---- INFO HEADER ----
clearinfo
writeInfoLine:  "=== SuperVP Transform v2.0 ==="
...
# Only for modes that actually use them.
needF0 = 0
needIntensity = 0
...
if fileReadable(resultWavFile$)
    ...
    if draw_visualization
        ...
        # F0 contour (modes that use pitch analysis)
        if needF0
            selectObject: sound1
            To Pitch: 0, 75, 600
...
appendInfoLine: ""
appendInfoLine: "=== SuperVP Transform complete ==="
