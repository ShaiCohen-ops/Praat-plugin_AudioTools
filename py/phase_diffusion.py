...
CORRECTNESS / RELIABILITY:
  - Preserve_transients now uses true TEMPORAL spectral flux (positive
    frame-to-frame magnitude change). v6.2 accidentally used np.diff(mag),
    i.e. differences between neighbouring frequency bins inside one frame.
  - Transient protection now also reduces the random-phase wet contribution,
    not only model-specific magnitude processing. Protected frames are locally
    blended back toward the dry STFT reconstruction before the global mix.
  - AE mel-band error -> FFT-bin projection is coverage-normalised. A flat
    mel-band error profile now maps to a neutral flat FFT profile instead of
    inheriting triangular-filter overlap geometry or being min-max exaggerated.
  - The Latent decoded mel envelope uses the same coverage-normalised
    projection, removing a systematic filterbank-overlap spectral tilt while
    retaining the existing energy-shaped timbral mapping.
  - AR(1) fitting now samples ADJACENT frame pairs when long files are capped
    to 512 pairs. v6.2 subsampled frames first, then treated distant samples as
    one-step neighbours.
  - K-means++ handles identical/degenerate latent vectors; short files may use
    a single cluster instead of crashing. Event fallback always yields at
    least one event for non-empty audio and covers the trailing remainder.
  - Removed the SciPy dependency from onset thresholding (same nearest-edge
    moving mean/std implemented in NumPy).
  - Spectral blur preserves flat edge magnitudes and caps its kernel to the
    available FFT-bin count, so extreme Mag_smear values cannot change array
    length or crash.
  - Analysis channel for multichannel input is now the strongest-RMS channel,
    rather than always channel 1; processing remains multichannel with
    independent phase realisations.
  - DC and Nyquist bins retain their original real values during phase
    randomisation.
  - Latent envelope exponentiation is numerically clipped to prevent overflow
    for extreme custom Temperature values.
...
─────────────────────────────────────────────────────────────────────────────
  ...
  latent — Full latent-space diffusion.
          ...
          event's Z is walked toward a temperature-weighted mixture of
          cluster attractors via annealed gradient descent + Langevin-style
          noise, then decoded
          back to a mel patch. The decoded patch's energy profile becomes
          a coverage-normalised spectral envelope for that event's window.
...
─────────────────────────────────────────────────────────────────────────────
  ...
  --mag-smear        FLOAT       all models: frequency-domain spectral blur
...
def _uniform_filter1d_nearest(x, size):
    """NumPy-only moving average with nearest-edge padding."""
...
def segment_events(audio_mono, sr, event_min=EVENT_MIN, event_max=EVENT_MAX):
    ...
    win       = max(8, int(0.025 * sr))   # 25 ms window
    min_samp  = max(1, int(event_min * sr))
    max_samp  = max(min_samp, int(event_max * sr))

    # Temporal spectral flux: positive frame-to-frame magnitude change.
    prev_mag  = np.zeros(win // 2 + 1)
    flux      = []
    for pos in range(0, max(1, n - win + 1), hop):
        frame = np.zeros(win, dtype=np.float64)
        available = min(win, n - pos)
        if available > 0:
            frame[:available] = audio_mono[pos:pos + available]
        frame  *= np.hanning(win)
    ...
    # NumPy-only implementation avoids a SciPy runtime dependency.
    w_frames   = max(1, int(0.200 / 0.010))
    w_size     = 2 * w_frames + 1
    local_mean = _uniform_filter1d_nearest(flux, w_size)
    local_sq   = _uniform_filter1d_nearest(flux ** 2, w_size)
    local_std  = np.sqrt(np.maximum(local_sq - local_mean ** 2, 0.0))
    threshold  = local_mean + local_std

    # Collect onset frames. Require a local peak so a broad threshold crossing
    # does not create multiple candidate onsets.
    onset_samps = [0]
    last_onset  = 0
    for i in range(1, len(flux) - 1):
        if (flux[i] > threshold[i] and
                flux[i] >= flux[i - 1] and flux[i] > flux[i + 1]):
            samp = min(n, i * hop)
    ...
    # Fallback: too few onset-derived events -> fixed ~0.5 s events.
    # Always cover the complete non-empty signal; merge a short tail into the
    # preceding event rather than silently dropping it.
    if len(events) < 4:
        events = []
        ...
        while s < n:
            e = min(n, s + step)
            if e - s < min_samp and events:
                events[-1]["end_time"] = n / sr
                break
            events.append({"start_time": s / sr, "end_time": e / sr})
            s = e

    if not events:
        events = [{"start_time": 0.0, "end_time": n / sr}]
...
def kmeans_plus_plus(Z, k, n_iter=60, seed=42):
    ...
    n   = len(Z)
    if n == 0:
        raise ValueError("k-means requires at least one latent vector")
    k   = max(1, min(int(k), n))
    ...
    for _ in range(k - 1):
        ...
        total = float(np.sum(d_sq))
        if not np.isfinite(total) or total <= 1e-20:
            # Degenerate latent cloud (e.g. silence / repeated identical events).
            remaining = [i for i in range(n) if i not in chosen]
            idx = remaining[rng.randint(len(remaining))] if remaining else rng.randint(n)
        else:
            probs = d_sq / total
...
#   3. Project through mel_fb.T and DIVIDE by per-bin filterbank coverage,
#      giving a weighted average of the overlapping mel-band errors rather
#      than a geometry-dependent sum.
#   4. Normalise to [0, 1]; effectively flat profiles map to neutral 0.5.
#
...
#
# What changed from v5.x / v6.2: v5 broadcast a single scalar across all mel
# bands; v6.0 kept the per-band axis but still used an unnormalised transpose
# sum, so some residual variation could still follow filterbank coverage.
# v6.3 uses a coverage-normalised weighted average, so the final frequency
# profile is genuinely driven by the AE band errors.
# ═══════════════════════════════════════════════════════════════════════════
...
def ae_error_to_bin_weights(band_errs, events, mel_fb):
    ...
    # Project as a coverage-normalised weighted AVERAGE. Without the denominator,
    # even a perfectly flat band-error profile inherits the triangular filterbank
    # overlap geometry and becomes spuriously frequency-dependent.
    numerator = mel_fb.T.dot(band_avg)
    coverage  = np.sum(mel_fb, axis=0)
    valid     = coverage > 1e-12
    bin_weights = np.zeros_like(numerator)
    bin_weights[valid] = numerator[valid] / coverage[valid]

    # Fill uncovered edge bins from the nearest covered value, rather than
    # inventing artificial minima at DC / Nyquist solely because mel coverage
    # begins at 20 Hz and triangular endpoints can be zero.
    if np.any(valid):
        idx = np.arange(len(bin_weights))
        bin_weights[~valid] = np.interp(idx[~valid], idx[valid], bin_weights[valid])

    # Normalise to [0, 1], but do not min-max-stretch an effectively flat
    # profile. With no frequency-discriminative evidence, 0.5 is the neutral
    # prior (rather than 0, which would classify every bin as maximally tonal).
    w_min, w_max = float(bin_weights.min()), float(bin_weights.max())
    span = w_max - w_min
    mean_abs = abs(float(np.mean(bin_weights))) + 1e-12
    cv = float(np.std(bin_weights)) / mean_abs
    scale = max(1.0, abs(w_min), abs(w_max))
    if span > 1e-10 * scale and cv > 0.02:
        bin_weights = (bin_weights - w_min) / span
    else:
        bin_weights = np.full_like(bin_weights, 0.5)
...
def latent_diffusion_step(z, centers, variances, temperature,
    ...
    """Single annealed gradient step toward a soft mixture of cluster centers."""
...
def diffuse_latent_z(z_orig, centers, variances, n_steps, temperature,
    ...
    """
    Walk z_orig toward temperature-weighted cluster attractors over n_steps,
    then blend with the original z by diffusion_amount.
...
def spectral_blur(mag, smear):
    """Frequency-domain smoothing with edge-preserving padding.

    `smear` has one meaning across pca / ar / latent. The odd Hann-kernel width
    grows with smear but is capped to the available FFT-bin count, so extreme
    custom values cannot change the output length.
    """
    mag = np.asarray(mag, dtype=np.float64)
    if smear <= 1e-6 or len(mag) < 3:
        return mag.copy()
    w = int(1 + 2 * int(round(smear * 2.0)))   # smear 1 -> 5 bins, 2 -> 9, ...
    max_w = len(mag) if len(mag) % 2 == 1 else len(mag) - 1
    w = min(w, max_w)
    if w <= 1:
        return mag.copy()
...
def temporal_spectral_flux(mag, prev_mag):
    """Positive frame-to-frame spectral flux (not frequency-bin slope)."""
    return float(np.sum(np.maximum(mag - prev_mag, 0.0)))


def _random_phase_spectrum(magnitude, rng, dc_value, nyquist_value):
    """Randomise interior phases while preserving real DC/Nyquist bins."""
    n_bins = len(magnitude)
    phase = rng.uniform(0.0, 2.0 * np.pi, n_bins)
    spec = magnitude * np.exp(1j * phase)
    spec[0] = complex(float(dc_value), 0.0)
    if n_bins > 1:
        spec[-1] = complex(float(nyquist_value), 0.0)
    return spec
...
def diffuse_frame_pca(magnitude, ae_weight, eff_amount, mag_smear, rng,
    ...
    return _random_phase_spectrum(mag_wet, rng, dc_value, nyquist_value)
...
def fit_ar_coefficients(frames_mag, max_pairs=512):
    """Fit per-bin AR(1) coefficients from true adjacent frame pairs."""
    n = len(frames_mag)
    if n == 0:
        raise ValueError("AR fit requires at least one magnitude frame")
    if n < 3:
    ...
    if n_pairs <= max_pairs:
        ...
        pair_idx = np.unique(
            np.linspace(0, n_pairs - 1, max_pairs, dtype=int)
        )

    num = np.zeros_like(frames_mag[0], dtype=np.float64)
    den = np.zeros_like(frames_mag[0], dtype=np.float64)
    for i in pair_idx:
        x = np.asarray(frames_mag[i], dtype=np.float64)
        y = np.asarray(frames_mag[i + 1], dtype=np.float64)
        num += x * y
        den += x * x
...
def diffuse_frame_ar(magnitude, ar_coeff, ae_weight, eff_amount, mag_smear,
                     smear_state, rng, dc_value, nyquist_value):
    """
...
def build_latent_mag_envelope(model, sr, win_size, z_diffused, mel_fb_win=None):
    ...
    built at win_size resolution -> energy-derived spectral envelope.
    """
    ...
    mel_energy   = np.exp(np.clip(mel_energy, -60.0, 60.0))
    mel_energy   = np.maximum(mel_energy, 1e-10)
    # Coverage-normalised transpose projection. This is still an
    # energy-derived spectral envelope (not a literal inverse-STFT magnitude),
    # but a flat decoded mel profile now maps to a flat FFT profile instead of
    # inheriting triangular-filter overlap geometry.
    numerator = mel_fb_win.T.dot(mel_energy)
    coverage  = np.sum(mel_fb_win, axis=0)
    valid     = coverage > 1e-12
    fft_envelope = np.zeros_like(numerator)
    fft_envelope[valid] = numerator[valid] / coverage[valid]
    if np.any(valid):
        idx = np.arange(len(fft_envelope))
        fft_envelope[~valid] = np.interp(
            idx[~valid], idx[valid], fft_envelope[valid])
...
def process_channel(signal, sr, model, ae_weights, events, Z,
    ...
    # ── Pass 1: collect magnitudes + TRUE temporal spectral flux ───────
    mag_frames      = []
    edge_bins       = []   # original real DC / Nyquist values
    flux_frames     = []
    frame_positions = []
    prev_mag = None
    pos = 0
    while pos + win_size <= len(x):
        ...
        edge_bins.append((float(spec[0].real), float(spec[-1].real)))
        if prev_mag is None:
            flux_frames.append(0.0)
        else:
            flux_frames.append(temporal_spectral_flux(mag, prev_mag))
        prev_mag = mag
    ...
    if model_name == "pca":
        ...
        ar_coeff = fit_ar_coefficients(mag_frames, max_pairs=512)
        smear_st = [None]
        if debug and ch == 0:
    ...
    for fi, pos in enumerate(frame_positions):
        ...
        transient_wet_ratio = 1.0
        if preserve_transients and flux_thr > 0.0 and flux_arr[fi] > flux_thr:
            excess = flux_arr[fi] / (flux_thr + 1e-12)
            transient_wet_ratio = 1.0 / max(1.0, excess)
            eff_amount = diffusion_amount * transient_wet_ratio

        dc_value, nyquist_value = edge_bins[fi]

        if model_name == "pca":
            new_spec = diffuse_frame_pca(
                mag, ae_weights, eff_amount, mag_smear, rng,
                dc_value, nyquist_value)

        elif model_name == "ar":
            new_spec = diffuse_frame_ar(
                mag, ar_coeff, ae_weights, eff_amount, mag_smear, smear_st, rng,
                dc_value, nyquist_value)
            ...
            new_spec = diffuse_frame_latent(
                mag, latent_envelopes[ev_idx], eff_amount, mag_smear, rng,
                dc_value, nyquist_value)

        wet_frame = np.fft.irfft(new_spec, n=win_size) * win

        # Protect transient phase as well as magnitude. v6.2 reduced only the
        # model's magnitude parameter while still feeding fully random phase
        # into the global wet mix. A local dry STFT reconstruction makes the
        # protection audible and faithful to the control's name.
        if transient_wet_ratio < 1.0:
            dry_frame = x[pos:pos + win_size] * (win ** 2)
            out_frame = (transient_wet_ratio * wet_frame +
                         (1.0 - transient_wet_ratio) * dry_frame)
        else:
            out_frame = wet_frame
...
def main():
    parser = argparse.ArgumentParser(
        description="Latent Spectral Diffusion Engine v6.3 -- Praat AudioTools"
    )
    ...
    win_size         = max(256, int(args.window_size))
    if win_size % 2:
        win_size += 1   # keep an explicit Nyquist bin for phase-edge handling
    hop_size         = max(1,   min(int(args.hop_size), win_size // 2))
    ...
    if n_samples == 0:
        print("ERROR: input audio contains no samples", file=sys.stderr)
        sys.exit(1)
    ...
    # ── Stage 1: Segment ─────────────────────────────────────────────
    # Use the strongest RMS channel as the analysis reference. This avoids a
    # silent/weak channel 1 driving all learned weights on multichannel files,
    # while keeping the actual processing multichannel.
    ch_rms = np.sqrt(np.mean(audio ** 2, axis=0))
    analysis_ch = int(np.argmax(ch_rms))
    audio_mono = audio[:, analysis_ch]
    if args.debug:
        print("  [Stage 1] Segmenting audio on analysis channel %d/%d..."
              % (analysis_ch + 1, n_channels))
    events = segment_events(audio_mono, sr)
    if not events:
        print("ERROR: event segmentation produced no events", file=sys.stderr)
        sys.exit(1)
    if args.debug:
    ...
    # ── Stage 4b: Cluster ─────────────────────────────────────────────
    k_actual = max(1, min(n_clusters, len(events)))
    if args.debug:
    ...
    print("Latent Spectral Diffusion OK | v6.3 | model=%s | amount=%.3f | "
