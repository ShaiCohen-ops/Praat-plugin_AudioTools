...
tempInput$  = pluginDir$ + "paulstretch_input.wav"
tempOutput$ = pluginDir$ + "paulstretch_output.wav"
...
# ---- INFO ----
clearinfo
writeInfoLine:  "=== Paulstretch v2.0 ==="
...
# ---- CALL PYTHON ----
appendInfoLine: "[2/4] Running Paulstretch (this may take a while)..."
...
if not fileReadable(tempOutput$)
    ...
    exitScript: "Python Paulstretch failed." + newline$
...
if draw_visualization
    ...
    # Compute original spectrum centroid trajectory via intensity
    # (intensity is a good proxy for energy envelope in this context)
    selectObject: sound
    if nChannels > 1
        Extract one channel: 1
    ...
    # Get intensity range for shared axes
    ...
    # Draw both on normalized time axis [0, 1]
    Axes: 0, 1, intMin, intMax
    Paint rectangle: "{0.97, 0.97, 0.98}", 0, 1, intMin, intMax

    # Original (grey, compressed to left portion)
