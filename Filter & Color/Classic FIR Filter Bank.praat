...
if apply_filter
    ...
    if dry_wet_mix < 1
        ...
        if numChannels > 1
            ...
            dryMono = Copy: "dry_" + uniqueID$
        endif
        
        selectObject: filtered
        if numChannels > 1
            wetMono = Convert to mono
        else
            wetMono = Copy: "wet_" + uniqueID$
        endif
        
        # For stereo, we need to handle differently
        if numChannels = 1
            selectObject: filtered
            Formula: "'dry_wet_mix' * self + (1 - 'dry_wet_mix') * Sound_dry_'uniqueID$'(x)"
        else
            selectObject: sound
            dry = Copy: "dryStereo_" + uniqueID$
            
            selectObject: filtered
            durFilt = Get total duration
            
            selectObject: dry
            durDry = Get total duration
            
            # Match durations
            if durFilt <> durDry
                selectObject: dry
                Extract part: 0, durFilt, "rectangular", 1, "no"
                dryTrimmed = selected("Sound")
                removeObject: dry
                dry = dryTrimmed
            endif
            
            selectObject: filtered
            Formula: "'dry_wet_mix' * self + (1 - 'dry_wet_mix') * Sound_dryStereo_'uniqueID$'(x)"
            
            removeObject: dry
        endif
        
        removeObject: dryMono, wetMono
    endif
