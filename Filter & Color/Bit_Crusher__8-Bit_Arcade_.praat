...
# Version: 0.3 (2025) - Real SR-reduction (sample-and-hold) + spectral length fix
# License: MIT License
...
procedure processChannel: .inputSound
    ...
    if mode = 1
        ...
        # Apply sample rate reduction via SAMPLE-AND-HOLD decimation.
        # This holds each block of N samples, producing the aliased, gritty
        # stair-step of a real bit-crusher. (Praat's Resample is anti-aliased,
        # so down+up resampling would only low-pass / muffle the signal.)
        if sample_rate_reduction > 1
            srRed$ = string$(round(sample_rate_reduction))
        ...
        # To Spectrum: "yes" zero-pads to a power of 2, so To Sound is longer
        # than the input -- trim back to the original duration.
